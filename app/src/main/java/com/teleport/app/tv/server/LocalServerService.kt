                            ...
                            try {
                                for (frame in incoming) {
                                    if (frame is Frame.Binary) {
                                        // Bolt: Use `frame.data` directly instead of `frame.readBytes()` to avoid
                                        // redundant ByteArray allocations and reduce GC pressure during high-frequency mirroring
                                        val data = frame.data
