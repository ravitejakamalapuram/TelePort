...
## 2024-07-20 - Expand Touch Targets for List Toggles in Web Extensions
**Learning:** In a dense list of settings (like the Chrome extension popup), users struggle to tap small custom toggle switches. Wrapping the entire row (title, description, and switch) in a single `<label>` linked to the underlying `<input>` via the `for` attribute significantly increases the hit area (Fitts's Law) and provides better context for screen readers.
**Action:** Always wrap the entire row configuration inside a `<label>` instead of just wrapping the custom toggle UI, and apply `cursor: pointer` to the row to signal interactability.
