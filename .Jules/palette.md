...
## 2024-06-23 - Auto-dismiss keyboard after explicit action
**Learning:** In Jetpack Compose, text inputs often retain focus after a user triggers a software keyboard action (like "Go" or "Send") or clicks a dedicated UI submission button, leaving the keyboard open and obscuring the UI.
**Action:** Always retrieve `val focusManager = LocalFocusManager.current` and call `focusManager.clearFocus()` within both the `KeyboardActions` callback and the corresponding explicit submit button`s `onClick` handler.
