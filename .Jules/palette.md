...
## 2026-07-27 - Make Text-Based Links Accessible
**Learning:** Standalone Text elements styled as links (like 'Terms of Service' with underlines) do not natively convey their interactivity to screen readers, nor do they provide the standard visual feedback (touch ripples) when tapped. This makes them difficult to discover and use for both visually and motor-impaired users.
**Action:** When implementing text-based links in Jetpack Compose, apply a structured sequence of modifiers: `.clip(RoundedCornerShape(4.dp))`, `.clickable(role = Role.Button) { ... }`, and `.padding(4.dp)` to the Text component. This ensures the touch target is adequately sized, the ripple effect is neatly constrained, and screen readers accurately announce the element as an interactive button.
