...
## 2026-07-02 - Filter High-Frequency Logging to Prevent GC Thrashing
**Learning:** Logging (e.g., `Log.d`) high-frequency commands like `Command.MoveCursor` or `Command.Scroll` inside event bus collectors causes severe performance degradation, logcat spam, and Garbage Collection (GC) thrashing due to continuous string allocation and string interpolation.
**Action:** Always wrap logging of such events in conditional checks (e.g., `if (command !is Command.MoveCursor && command !is Command.Scroll)`) to prevent unnecessary memory allocations in hot code paths.
