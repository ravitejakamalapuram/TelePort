...
## 2026-08-10 - Filter High-Frequency Logs to Reduce GC Pressure
**Learning:** String templates in `Log.d` (e.g., `"... $var"`) inside high-frequency flow collectors (like `TvEventBus.commands` for cursor and scroll events) are eagerly evaluated, causing continuous string allocations and GC thrashing even if logs are disabled or ignored.
**Action:** Always wrap logging statements for high-frequency events in conditional checks (e.g., `if (command !is Command.MoveCursor && command !is Command.Scroll)`) to prevent unnecessary allocations before the log statement is evaluated.
