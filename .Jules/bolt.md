...
## 2026-08-11 - Eager String Evaluation in Event Flows
**Learning:** String templates (e.g., "... $var") are eagerly evaluated. Using them inside high-frequency flow collectors (like TvEventBus.commands for cursor movement or scrolling) causes continuous string allocation and GC thrashing.
**Action:** Always wrap logging statements in flow collectors with conditional checks (e.g., if (command !is Command.MoveCursor)) to prevent unnecessary allocations, and ensure unhandled high-frequency events return early before eager operations occur.
