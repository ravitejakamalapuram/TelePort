...
## 2026-08-02 - Prevent Logcat GC Thrashing from High-Frequency Broadcasts
**Learning:** High-frequency events (like 100Hz `MoveCursor` or `Scroll` commands) broadcasted via `TvEventBus` trigger eager string interpolations (e.g., `Log.d(TAG, "Command: $cmd")`) in all collectors. The continuous string allocation and logcat I/O overhead causes GC thrashing and UI stuttering.
**Action:** When working with high-frequency shared streams, filter out noisy events at the very top of `collectLatest` blocks before applying any general-purpose logging or string formatting.
