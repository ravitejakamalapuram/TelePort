...
## 2026-07-02 - Filter High-Frequency Logging overhead
**Learning:** In event-driven systems like `TvEventBus.commands.collectLatest`, blindly logging every command, especially high-frequency ones like `MoveCursor` and `Scroll` (which can fire at 100Hz+), causes significant CPU overhead for string formatting, excessive IPC calls to logcat, and fills up memory buffers leading to GC churn and dropped frames.
**Action:** Always filter out high-frequency events when adding general-purpose debug logging in event pipelines to preserve performance and prevent logcat spam.
