...
## 2024-08-09 - Avoid eager logging for high-frequency Flow events
**Learning:** Even if a high-frequency command (like scrolling or cursor movement) isn't explicitly handled or if logging is ultimately unwanted for it, unconditional string interpolation (e.g. `Log.d(TAG, "Command: $command")`) at the top of a `collectLatest` block eagerly allocates strings, causing massive GC pressure.
**Action:** Always filter out high-frequency events at the top of Flow collectors using conditionals before any logging or object allocation occurs.
