...
## 2026-08-04 - Eliminate Eager Logging Allocations in High-Frequency Flow Collectors
**Learning:** String templates in `Log.d` statements inside `collectLatest` blocks for high-frequency global event buses (like `TvEventBus.commands`) cause continuous string allocation and GC thrashing for commands like `MoveCursor` and `Scroll` (100Hz+), even if the logger's output isn't closely monitored.
**Action:** Always wrap logging statements in conditional checks (`if (command !is Command.MoveCursor && command !is Command.Scroll)`) when consuming global, high-frequency flows to prevent unnecessary allocations.
