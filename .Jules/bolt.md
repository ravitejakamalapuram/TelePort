...
## 2026-07-02 - Filter High-Frequency Events Before Logging
**Learning:** String templates (e.g., `Log.d(TAG, "Command: $command")`) are eagerly evaluated. Using them inside high-frequency event collectors (like MoveCursor at 60Hz+) causes continuous string allocation and GC thrashing.
**Action:** Always wrap logging statements in conditional checks (e.g., `if (event !is HighFrequencyEvent)`) to prevent unnecessary allocations for high-frequency events.
