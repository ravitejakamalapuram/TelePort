...
## 2026-08-05 - Avoid Eager String Allocation and Re-composition in Event Collectors
**Learning:** String templates inside high-frequency event flow collectors (like `collectLatest` for 100Hz hardware cursor events) cause eager string allocation and severe GC pressure, even if logging is disabled. Furthermore, propagating unhandled high-frequency events causes unnecessary `collectLatest` block cancellations.
**Action:** Wrap logging statements in conditionals (`if (command !is HighFrequencyEvent)`) and early return unhandled high-frequency events (`if (command is UnhandledEvent) return@collectLatest`) at the top of collectors.
