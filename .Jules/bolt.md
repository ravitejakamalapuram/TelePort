...
## 2026-08-08 - Prevent eager string formatting in high-frequency event flow collectors
**Learning:** String templates in logging statements (e.g., `Log.d(TAG, "... $command")`) are eagerly evaluated. When placed directly inside high-frequency flow collectors (like `TvEventBus.commands.collectLatest` which receives 100Hz+ `MoveCursor` or `Scroll` events), they cause continuous string allocation and GC thrashing.
**Action:** Always wrap logging statements inside high-frequency event handlers with conditional checks (e.g., `if (command !is Command.MoveCursor && command !is Command.Scroll)`) to prevent unnecessary allocations.
