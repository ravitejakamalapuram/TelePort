...
## 2026-08-01 - Avoid Logging Inside High-Frequency Event Callbacks
**Learning:** String templates (e.g., `"Executing command: $command"`) are eagerly evaluated. Using them inside high-frequency event flow collectors (like processing `MoveCursor` at 60Hz) causes continuous string allocation and severe GC thrashing, even if the output is ignored.
**Action:** Always wrap logging statements inside high-frequency flow collectors or event callbacks with conditional checks (e.g., `if (command !is HighFrequencyCommand)`) to filter out the noise and prevent unnecessary allocations.
