...
## 2026-07-20 - Avoid Log.d in High-Frequency Flow Collectors
**Learning:** In Jetpack Compose or standard coroutine flow collectors (like `TvEventBus.commands.collectLatest`), logging every emitted item with `Log.d` can cause severe GC pressure and logcat spam when processing high-frequency sensor events (like `MoveCursor` or `Scroll` commands arriving at 60-100Hz).
**Action:** Always filter out high-frequency event types before performing general-purpose logging (e.g., `if (command !is Command.MoveCursor && command !is Command.Scroll) { Log.d(...) }`) to eliminate string formatting overhead and IPC delays in hot paths.
