...
## 2026-07-29 - Eager String Evaluation in Logging
**Learning:** String templates (e.g., `"Executing command $command"`) in Kotlin are eagerly evaluated. Placing these inside high-frequency event loops or flow collectors (like processing 60Hz cursor events) causes severe string allocation overhead and GC thrashing, even if the logs are effectively ignored.
**Action:** Always wrap `Log.d` or other logging statements inside high-frequency loops with explicit conditional checks (e.g., `if (command !is Command.MoveCursor)`) to prevent unnecessary eager string interpolation.
