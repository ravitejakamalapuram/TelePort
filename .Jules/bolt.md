...
## 2026-07-02 - Avoid Logging High-Frequency Commands
**Learning:** Logging high-frequency commands (e.g., `Command.MoveCursor`, `Command.Scroll` at 60Hz+) in the Android TV event bus causes significant IPC overhead and string allocation pressure due to rapid logcat emissions.
**Action:** Always filter out high-frequency events when doing general-purpose debugging/logging within an event loop or broadcast receiver to prevent logcat spam and preserve application performance.
