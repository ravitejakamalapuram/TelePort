...
## 2026-08-07 - Prevent GC Thrashing from High-Frequency Log String Interpolation
**Learning:** String templates (e.g., `"... $var"`) are eagerly evaluated. Using them inside high-frequency flow collectors (like `Command.MoveCursor` or `Command.Scroll`) causes continuous string allocation and GC thrashing, even if the logger's output is suppressed.
**Action:** Always wrap such logging in conditional checks (e.g., `if (command !is HighFrequencyEvent)`) and filter unhandled events immediately at the top of the flow collector to prevent unnecessary allocations.
