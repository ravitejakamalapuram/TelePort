...
## 2026-07-25 - Filter High-Frequency Sensor Events from Logcat
**Learning:** Logging high-frequency sensor commands (like Cursor and Scroll movements at ~60Hz) directly to Logcat creates massive string interpolation overhead and IPC spam. This starves the main thread, leading to UI jank and garbage collection stuttering during smooth continuous motions.
**Action:** When working with high-frequency event streams, always explicitly filter out continuous/noisy events (e.g., `command !is Command.MoveCursor && command !is Command.Scroll`) before invoking `Log.d` to preserve main thread performance while maintaining debuggability for significant state changes.
