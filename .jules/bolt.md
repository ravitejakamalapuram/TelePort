...
## 2024-05-18 - Avoid frame.readBytes() in high-frequency Ktor WebSocket routes
**Learning:** In Ktor WebSockets, calling `frame.readBytes()` allocates a new `ByteArray` and copies the underlying data. For high-frequency message handling (like continuous binary streaming from a device mirror), this generates significant unnecessary garbage and causes GC pauses that lead to jitter.
**Action:** Access `frame.data` directly to get the underlying `ByteArray` without copying, saving CPU time and avoiding unnecessary allocations.
