...
## 2023-10-25 - Ktor WebSocket Frame Allocation
**Learning:** In Ktor WebSockets, `Frame.readBytes()` allocates a new `ByteArray` and copies the underlying data. For high-frequency message handling (e.g., streaming frames), this creates immense GC pressure.
**Action:** Access `Frame.data` directly instead of calling `Frame.readBytes()` when reading binary payload data to avoid redundant allocations.
