...
## 2024-11-20 - Redundant Array Copy in Ktor WebSockets
**Learning:** In Ktor WebSockets, `Frame.readBytes()` is an extension function that returns `data.copyOf()`. This creates a redundant array copy for every incoming frame, which adds significant memory and GC pressure in high-frequency scenarios like streaming video or screen mirroring.
**Action:** Replace `frame.readBytes()` with direct property access (`frame.data`) in hot paths to eliminate the array allocation and minor function call overhead, significantly reducing GC pressure without altering behavior.
