...
## 2024-05-14 - Redundant Array Copy in Ktor WebSocket Frame Reading
**Learning:** In Ktor WebSockets, the `Frame.readBytes()` extension function internally returns `data.copyOf()`. In high-frequency, large-payload scenarios like streaming video frames via WebSockets, this causes massive, continuous redundant byte array allocations, leading to severe GC pressure and potential stutter.
**Action:** When handling high-frequency binary frames where the frame is consumed immediately and no mutation happens downstream, directly access `frame.data` instead of calling `frame.readBytes()`.
