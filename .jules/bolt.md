...
## 2024-05-18 - Avoid redundant allocations in Ktor WebSockets
**Learning:** In Ktor WebSockets, `Frame.readBytes()` allocates a new `ByteArray` and copies the underlying data. For high-frequency message handling (e.g., streaming frames or mirroring), this creates unnecessary Garbage Collection (GC) overhead and CPU copying, which can bottleneck performance on low-end devices.
**Action:** When handling high-frequency binary frames in Ktor WebSockets, access the underlying bytes directly using `Frame.data` to avoid redundant allocations and reduce GC pressure.
