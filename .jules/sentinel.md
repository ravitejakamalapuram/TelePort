...
## 2026-06-03 - Scheme Whitelist Bypass via Leading Whitespace
**Vulnerability:** In `TabManager.kt`, URL sanitization parsed the scheme directly from the raw user-provided URL (`android.net.Uri.parse(url).scheme`). Android's `Uri.parse` ignores or incorrectly parses the scheme if the URL is prepended with whitespace (e.g., ` javascript:alert(1)`), causing `scheme` to evaluate to `null`. This bypassed the explicit scheme whitelist (`http`, `https`, `about`, `data`).
**Learning:** `android.net.Uri` does not trim whitespace automatically before parsing. This creates a dangerous differential between the URL validation layer (which sees `null`) and the WebView execution layer (which may strip whitespace and execute the malicious scheme).
**Prevention:** Always call `.trim()` on URL strings from untrusted input before using `android.net.Uri.parse` to extract and validate the scheme.
